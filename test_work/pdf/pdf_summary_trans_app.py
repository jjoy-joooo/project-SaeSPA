...
import my_text_trans_sum # 텍스트를 요약하기 위한 모듈
...
# PDF 파일을 요약하는 함수
# def summarize_PDF_file(pdf_file, lang):
def summarize_PDF_file(pdf_file, lang, trans_checked):
    if (pdf_file is not None):
        ...
        for page in reader.pages:
            ...
            text_summary = my_text_trans_sum.summarize_text(page_text, lang)
            text_summaries.append(text_summary)
            
        token_num, final_summary = my_text_trans_sum.summarize_text_final(text_summaries, lang)
        ...
        if final_summary != "":
            ...
            if trans_checked:
                trans_result = my_text_trans_sum.traslate_english_to_korean_using_openAI(final_summary)
...
if clicked:
    # summarize_PDF_file(uploaded_file, lang_code) # PDF 파일 요약 수행
